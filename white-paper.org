...
characters. After another whitespace, there can be optional metadata which
...
for the label or includes metadata, while the # symbol initiates the full
expansion of the empty formula, turning it into its complete composition. It
must be placed before the formula begins.As shown in figure, [[fig:complex-formula]].
